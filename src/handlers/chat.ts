...
export async function chatHandler(req: Request, res: Response, config: GatewayConfig): Promise<void> {
  ...
  try {
    switch (providerConfig.type) {
      case 'openai-compatible':
        await openaiCompat.chat(req, res, model, providerConfig);
        break;
      case 'anthropic':
        await anthropicProvider.chat(req, res, model, providerConfig);
        break;
      case 'google':
        await googleProvider.chat(req, res, model, providerConfig);
        break;
      default:
    ...
    logRequest(req, res, startMs, { provider: providerKey, model, alias, stream: req.body?.stream === true });
  }
}
